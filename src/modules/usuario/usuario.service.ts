...
export class UsuarioService {
    ...
    async getUsuarioByEmail(params: GetUsuarioByEmailDto) {
        ...
        const lowerCaseEmail = params.correo.toLowerCase();

        const usuario = await this.usuarioRepository.findOne({
            where: {
                correo: ILike(lowerCaseEmail),
                id_estadousuario: Not(id_estadousuario)
            }
        });

        if (!usuario) {
            throw new NotFoundException(`Usuario with correo ${lowerCaseEmail} not found`)
        };
    ...
    async getUsuarios(params: GetUsuariosDto): Promise<UsuariosResponseDto> {
        ...
        const query = this.usuarioRepository
            ...
            .select(['u.id', 'u.nombre', 'u.correo', 'u.id_tipousuario', 'tu.nombre', 'eu.nombre'])
    ...
    async editUsuario(params: EditUsuarioDto): Promise<void> {
        ...
        if (existing) {
            throw new ConflictException(`Usuario "${params.nombre}" and email "${params.correo}" already exists`);
        };
    ...
    async createUsuario(params: CreateUsuarioDto): Promise<number> {
        ...
        let usuario = this.usuarioRepository.create({ correo: params.correo, nombre: params.nombre, contraseña: hash, id_tipousuario: params.id_tipousuario, id_estadousuario: id_estadousuario });
